# Hi 👋 I'm Shivam Kumar

### 🚀 AI Product Builder | Indie App Creator | Backend Developer

I love building products that solve real problems using **AI, automation, and software engineering**. Currently, I’m focused on **AI Product Management**, building my own apps, and exploring how technology can create meaningful user experiences.

I enjoy turning ideas into products — from **concept → market research → development → launch**.

---

### 🌍 About Me

* 🇮🇳 Based in India
* 💼 Backend Developer with experience in **Java, Spring Boot, Microservices & REST APIs**
* 🤖 Exploring **AI Product Management, AI-powered applications & automation**
* 📱 Building my own apps under **SK Forge Labs**
* 🚀 Passionate about turning ideas into scalable digital products
* 🤝 Open to collaborating on **AI, SaaS, Mobile Apps & Innovative Projects**
* ⚡ Engineer by profession, **Builder by passion**

---

### 🛠️ Tech Stack

#### Backend Development

`Java` `Spring Boot` `Microservices` `REST APIs` `PostgreSQL` `MySQL` `Hibernate/JPA`

#### Tools & Platforms

`Git` `GitHub` `Postman` `Swagger` `JUnit` `Mockito`

#### Currently Exploring

`Saas` `AI Product Management` `LLMs` `Prompt Engineering` `Product Strategy` `Mobile Apps` `Automation` 

---

### 🚀 What I’m Building

* 🤖 AI-powered applications
* 📱 Mobile apps under **SK Forge Labs**
* 💡 Product ideas solving real-world problems
* 🔥 Exploring how AI can supercharge traditional apps

---

### 📲 My Apps

🏢 **Developer Profile:**
[SK Forge Labs](https://play.google.com/store/apps/developer?id=SK+Forge+Labs&hl=en_IN)

---

### 📫 Connect With Me

📧 **Email:** [shivamkeshri7033@gmail.com](mailto:shivamkeshri7033@gmail.com)

💬 Open to conversations about:
**AI Products • Startups • App Building • Backend Development • Product Strategy**


### Visitors
![](https://komarev.com/ghpvc/?username=SavvyShivam)

### DevCard
<a href="https://app.daily.dev/SavvyShivam"><img src="https://api.daily.dev/devcards/4e1725aafbef460fa75e3932acb2c224.png?r=nnu" width="400" alt="Shivam Kumar's Dev Card"/></a>

